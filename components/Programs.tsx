...
export default function Programs() {
    return(
        <section className="py-20 bg-foreground/4" id="programs">
                    ...
                    {programs.map((program, index) => (
                        ...
                        key={program.title} className="group cursor-pointer">
                            ...
                            <CardContent className="p-6 pt-0">
                                ...
                                text-foreground group-hover:text-primary transition-colors">{program.title}</motion.h3>
                                <motion.p 
                                initial= {{opacity: 0, y: 6}}
                                whileInView={{opacity: 1, y:0}}
                                viewport={{once: true}}
                                transition={{duration:0.5, ease: "easeOut", delay: index * 0.3}}
                                className="text-muted-foreground mb-4 leading-relaxed">{program.description}</motion.p>
                                <div>
                                    <Link href="#contact">
                                    <Button variant="outline" className="w-full transition-colors
                                    duration-300 group-hover:bg-primary
                                    dark:group-hover:bg-primary
                                    group-hover:text-primary-foreground">Learn more</Button>
                                    </Link>
                                </div>
